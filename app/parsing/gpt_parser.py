...
def build_prompt(markdown_content):
    ...
    extracted = _smart_extract(markdown_content, max_chars=MAX_CONTENT_LENGTH)
...
def extract_retry_wait(error_message):
    ...
    if millis_match:
        return (float(millis_match.group(1)) / 1000) + 2
...
def parse_with_openai(prompt):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print(
                f"OpenAI {OPENAI_PRIMARY_MODEL} rate-limited; "
...
def parse_investor(markdown_content, source_url: str = "", firm_name: str = ""):
    ...
    if openai_client:
        try:
            ...
            time.sleep(OPENAI_PARSER_DELAY_SECONDS)
            return parsed
